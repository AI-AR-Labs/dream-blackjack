...
async function testToken() {
  ...
  try {
    ...
    if (userRes.status === 200) {
      ...
      console.log(
        `❌ Failed to authenticate: ${userRes.status} - ${userRes.data.message}`,
      );
      return;
    }
  } catch (e) {
  ...
  try {
    ...
    if (repoRes.status === 200) {
      ...
      console.log(
        `   Permissions: ${JSON.stringify(repoRes.data.permissions || {})}`,
      );
    } else {
      console.log(
        `❌ Cannot access repository: ${repoRes.status} - ${repoRes.data.message}`,
      );
    }
  } catch (e) {
  ...
  try {
    const issueRes = await makeRequest(
      "/repos/AI-AR-Labs/dream-blackjack/issues/3",
    );
    if (issueRes.status === 200) {
      console.log(`✅ Can access issue #3: ${issueRes.data.title}`);
    } else {
      console.log(
        `❌ Cannot access issue: ${issueRes.status} - ${issueRes.data.message}`,
      );
      if (
        issueRes.status === 403 &&
        issueRes.data.message === "Resource not accessible by integration"
      ) {
  ...
  console.log("\n📊 Summary:");
  console.log(
    'If you see "Resource not accessible by integration" errors, it usually means:',
  );
  console.log(
    '1. The token doesn\'t have the right scopes (needs "repo" and "workflow")',
  );
  console.log("2. The token owner doesn't have access to the repository");
  console.log(
    "3. The repository has restricted permissions for GitHub Apps/tokens",
  );
}

if (TOKEN === "YOUR_BOT_TOKEN_HERE") {
  console.log(
    "❌ Please edit this file and replace YOUR_BOT_TOKEN_HERE with your actual token!",
  );
  process.exit(1);
}
